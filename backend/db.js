// backend/db.js
// طبقة الاتصال بقاعدة البيانات — بتستخدم الجداول الحقيقية
// (products, clients, invoices, invoice_items, payments) بدل الـ JSON الواحد.
...
function getPool() {
  if (!pool) {
    if (!process.env.DATABASE_URL) {
      throw new Error('DATABASE_URL مش موجود. راجع ملف .env (محليًا) أو Environment Variables (على Vercel).');
    }
    pool = new Pool({
...
async function createInvoice(clientId, { items, rawText }) {
  ...
  try {
    ...
    await client.query(
      'INSERT INTO invoices (id, client_id, date, total, raw_text) VALUES ($1,$2,$3,$4,$5)',
      [invoiceId, clientId, now, total, rawText || null]
    );
    for (const it of itemRows) {
      await client.query(
        'INSERT INTO invoice_items (id, invoice_id, product_id, product_name, quantity, unit_price, subtotal) VALUES ($1,$2,$3,$4,$5,$6,$7)',
        [it.id, invoiceId, it.productId, it.productName, it.quantity, it.unitPrice, it.subtotal]
      );
    }
    await client.query('COMMIT');
    return {
      id: invoiceId, date: now.toISOString(), total, rawText: rawText || '',
...
async function createPayment(clientId, { amount, note }) {
  ...
  const now = new Date();
  await getPool().query(
    'INSERT INTO payments (id, client_id, date, amount, note) VALUES ($1,$2,$3,$4,$5)',
    [id, clientId, now, amt, note || '']
  );
...
async function getFullState() {
  ...
  for (const row of itemsR.rows) {
    (itemsByInvoice[row.invoice_id] ||= []).push({
      ...
      quantity: numOr0(row.quantity), unitPrice: numOr0(row.unit_price), subtotal: numOr0(row.subtotal),
    });
  }
  const invoicesByClient = {};
  for (const row of invoicesR.rows) {
    (invoicesByClient[row.client_id] ||= []).push({
      id: row.id, date: row.date.toISOString(), total: numOr0(row.total),
      rawText: row.raw_text || '', items: itemsByInvoice[row.id] || [],
    });
  }
  const paymentsByClient = {};
  for (const row of paymentsR.rows) {
    (paymentsByClient[row.client_id] ||= []).push({
      id: row.id, date: row.date.toISOString(), amount: numOr0(row.amount), note: row.note || '',
    });
  }
